...
def get_group_members(cursor=Depends(get_db_cursor)):
...
def get_member(id: int, cursor=Depends(get_db_cursor)):
...
def add_group_member(group_member: GroupMemberCreate, cursor=Depends(get_db_cursor)):
    try:
        cursor.execute(
...
def delete_member(group_id: int, user_id: int, cursor=Depends(get_db_cursor)):
    cursor.execute(
