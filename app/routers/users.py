...
@router.get("/users/{id}")
def get_user(id: int, cursor = Depends(get_db_cursor)):
...
def create_user(user: UserCreate, cursor = Depends(get_db_cursor)):
    try:
        cursor.execute(
...
@router.put("/users/{id}")
def update_user(id: int, user: UserCreate, cursor = Depends(get_db_cursor)):
    cursor.execute(
...
def delete_user(id: int, cursor = Depends(get_db_cursor)):
    cursor.execute(
