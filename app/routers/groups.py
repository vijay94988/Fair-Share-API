...
@router.get("/groups")
def get_groups(cursor=Depends(get_db_cursor)):
...
def get_group(id: int, cursor=Depends(get_db_cursor)):
...
def create_group(group: GroupCreate, cursor=Depends(get_db_cursor)):
    try:
        cursor.execute(
...
def update_group(id: int, group: GroupCreate, cursor=Depends(get_db_cursor)):
    cursor.execute(
...
def delete_group(id: int, cursor=Depends(get_db_cursor)):
    cursor.execute(
